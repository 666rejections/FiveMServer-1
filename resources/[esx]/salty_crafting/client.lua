		...
		while true do
			...
			if inDrawingRange then
				...
				if not display and isPlayerInRange(GetEntityCoords(PlayerPedId()), Config.Shop.shopCoordinates, Config.Shop.zoneSize.x) then
					...
					AddTextComponentString("Drücke ~INPUT_CONTEXT~ um Gegenstände herzustellen")
...
AddEventHandler('esx_weashop:clipcli', function()
  ...
  if IsPedArmed(ped, 4) then
    ...
    if hash~=nil then
      ...
      ESX.ShowNotification("Waffe wurde geladen")
    else
      ESX.ShowNotification("Sie haben keine Waffe in der Hand")
    end
  else
    ESX.ShowNotification("Diese Art von Munition ist nicht geeignet")
  end
end)