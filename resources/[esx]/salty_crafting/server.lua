...
AddEventHandler('salty_crafting:craftItem', function(ingredients)
	...
	if not item then
		TriggerClientEvent('esx:showNotification', _source, 'Es wurde kein Rezept dafür gefunden')
	else
		if xPlayer ~= nil then
			if hasAllIngredients(xPlayer.inventory, Config.Recipes[item]) then
				...
				TriggerClientEvent('esx:showNotification', _source, '~y~Gegenstand wurde hergestellt: ~w~' .. itemLabel(item, xPlayer.inventory))
			else
				TriggerClientEvent('esx:showNotification', _source, 'Du hast nicht alle Materialien')
			end
		end
	end
end)
