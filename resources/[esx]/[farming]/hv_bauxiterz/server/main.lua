...
RegisterServerEvent('esx_drugs:pickedUpBauxi')
AddEventHandler('esx_drugs:pickedUpBauxi', function()
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.canCarryItem('baux', 1) then
		xPlayer.addInventoryItem('baux', 1)
	else
...
RegisterServerEvent('esx_drugs:processBauxi')
AddEventHandler('esx_drugs:processBauxi', function()
	if not playersProcessingCannabis[source] then
		...
		playersProcessingCannabis[_source] = ESX.SetTimeout(Config.Delays.WeedProcessing, function()
			...
			elseif xEisenerz.count < 3 then
				...
				xPlayer.removeInventoryItem('baux', 3)
				xPlayer.addInventoryItem('baux', 1)
