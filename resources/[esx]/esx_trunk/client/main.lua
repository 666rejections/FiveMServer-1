				...
				if (not IsTrunkOpen) then
					...
					if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
						...
						if locked == 1 or class == 15 or class == 16 or class == 14 then
							...
							ESX.ShowNotification('Trunk is ~r~locked')
						end
					end
				else 
					ESX.ShowNotification('~r~Trunk is allready open from another Person')
				end
			end, targa)
		else
          ...
          --fin test
            ...
            if not max then
              --  VehicleMaxSpeed(vehFront,poid,Config.VehicleLimit[GetVehicleClass(vehFront)])
			   ...
			   local totalweight = tonumber(weight) - Itemweight
			   local Kgweight =  totalweight/1000
			   ESX.ShowNotification('Maximum Weight : ~g~'.. Kgweight .. ' Kg / '..MaxVh..' Kg')

            else
              ESX.ShowNotification('~r~ Too Heavy')
            end
			    else
			      ESX.ShowNotification('~r~ Wrong')
			    end
